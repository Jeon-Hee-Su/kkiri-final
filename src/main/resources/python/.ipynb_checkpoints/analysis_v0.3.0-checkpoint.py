...
def get_db_data(group_id):
    ...
    try:
        # pandas가 요구하는 SQLAlchemy 엔진으로 연결
        ...
        with engine.connect() as conn:
            df = pd.read_sql(query, conn, params={"group_id": int(group_id)})
...
# Java에서 그룹별 파일 경로를 두 번째 인자로 전달 (없으면 기본값)
result_json_path = sys.argv[2] if len(sys.argv) > 2 else "result.json"
...
if df.empty:
    ...
    # 카테고리명의 앞뒤 공백을 제거하여 JS 컬러맵과 일치시킴
    ...
    final_result = {
        ...
        "dataValues": [max(1, round((amt/total_amount)*100)) for amt in category_sum.values],
        "aiHeadline": accum_h,
